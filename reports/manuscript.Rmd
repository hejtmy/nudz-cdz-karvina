...
temp_mixed_func <- function(varname){
  out <- df_rbans_long %>%
    filter(subtest == varname) %>%
    lmer(z_score ~ diagnosis + session + (1|rbansid), data = .)
  return(out)
}
...
knitr::kable(as.data.frame(res),
             caption = "Diagnosis as a predictor of RBANS performance while controlling for the effect of participant and session.
             Coefficients are for schizophrenia in contrast to MDD (negative = schizophrenia does worse)")
