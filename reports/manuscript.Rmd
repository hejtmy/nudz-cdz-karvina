...
We have observed that patients suffering from schizophrenia had significantly worse baseline RBANS performance than patietns with affective disorders (see table XXX). This was one of the reasons we opted for the linear mixed modelling in our subsequent analyses as they should be able to control for the effect of diagnosis.
...
df_diag_difference <- df_rbans_long %>%
  filter(grepl("dim_", subtest)) %>%
  group_by(subtest) %>%
  summarise(t_test = apa_print(t.test(z_score~diagnosis))$statistic, 
            .groups = "drop")

df_rbans_long %>%
  filter(grepl("dim_", subtest), session == 1) %>%
  group_by(subtest, diagnosis) %>%
  summarise(m = mean_and_sd_report(z_score, type = "parentheses")) %>%
...
n_vr <- sum(!is.na(df_questionnaire$vr_benefit))
...
df_rbans %>%
  ...
  summarise_at(vars(starts_with("dim")), list(
...
df_rbans_long %>%
  ...
  mutate(session = paste0("session ", session - 1),
         fac_subtest = factor(subtest, levels = c("dim_celek", "dim_kratkodobapamet",
                                                  "dim_dlouhodobapamet", "dim_rec",
                                                  "dim_pozornost", "dim_vizuoprostor"))) %>%
  ggplot(aes(fac_subtest, z_score, fill = trenink)) +
    geom_boxplot() +
    facet_wrap(~session,  strip.position = "bottom") +
    scale_x_discrete(labels = c("dim_celek" = "Total", "dim_kratkodobapamet" = "Immediate memory", 
    "dim_vizuoprostor" = "Visuospatial abilities", "dim_rec" = "Speech", "dim_pozornost" = "Attention",
    "dim_dlouhodobapamet" = "Long term memory")) +
...
res <- sapply(dims, function(x){report_mixed_effect(temp_mixed_func(x), 2)})
knitr::kable(as.data.frame(res),
             caption = "Trenink as a predictor of RBANS performance. Coefficients are for VR in contrast to TP (negative = VR does worse)")
...
df_rbans_long %>%
  ...
  ggplot(aes(session, z_score, fill = trenink)) +
    ...
    scale_fill_discrete(labels = c("baseline", "TP" = "Standard treatment", "VR" = "VEs program")) +
