...
lmer_ratio_session <- df_results_remediation %>%
  lmer(correct_ratio ~ session + n_items + (1|rbansid), data = .)
summary(lmer_ratio_session)
```
...
df_rbans_long %>%
  ...
  ggplot(aes(fac_subtest, score, fill = trenink)) +
    ...
    labs(x = "", y = "RBANS IQ score",
         title = "IQ scores of RBANS cognitive domains",
         fill = "")
...
df_rbans_long %>%
  ...
  ggplot(aes(subtest, score, fill = trenink)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          legend.position = c(0.5,1), legend.direction = "horizontal") +
      labs(x = "", y = "RBANS IQ score",
         title = "IQ scores of RBANS cognitive domains after each intervention",
         fill = "")
